...
<body>
	...
	<form:form modelAttribute="formCmd" method="POST"
		action="${formActionUrl}" id="form1" enctype="multipart/form-data">

		<form:hidden path="formId" />

		<c:if test="${formAction == 'formDesign'}">
			<div style="float: right; margin: 2px; padding: 2px;">
				<span class="help"> <select name="newFieldType0">
				...
				</select> <a href="#" onclick="newField('${formCmd.formId}',0,0);"><fmt:message
							key="action.add" /></a>
				</span>
			</div>
			<div class="label"></div>
		...
		<c:forEach var="f" items="${formCmd.fields}" varStatus="status">
			...
			<c:if test="${formAction == 'formDesign'}">
				<div style="float: left; margin: 2px; padding: 2px;">
					<span class="help"> <a href="#"
						onclick="editField('${formCmd.formId}','${f.fieldId}');"> <fmt:message
								key="action.edit" />
					</a> | <a href="#"
						onclick="deleteField('${formCmd.formId}','${f.fieldId}');"> <fmt:message
								key="action.delete" />
					</a> | <c:if test="${not status.first}">
							<a href="#"
								onclick="moveField('${formCmd.formId}','${f.fieldId}',${f.fieldOrder-1});">
								<fmt:message key="action.moveUp" />
							</a> |
                    </c:if> <c:if test="${not status.last}">
							<a href="#"
								onclick="moveField('${formCmd.formId}','${f.fieldId}',${f.fieldOrder+1});">
				...
				<div style="float: right; margin: 2px; padding: 2px;">
					<span class="help"> <select name="newFieldType${indx+1}">
							<option value="text">Text</option>
							<option value="select">Select</option>
							<option value="checkbox">Checkbox</option>
							<option value="radio">Radio</option>
						...
						onclick="newField('${formCmd.formId}',${indx+1},${f.fieldOrder+1});">
			...
			<c:if test="${f.type == 'textarea'}">
				...
				<div class="field">
					<form:textarea path="fields[${indx}].strVal"
						cssClass="${f.cssClass}" />
					<form:errors path="fields[${indx}].strVal" cssClass="error" />
				</div>
				<div class="clear"></div>
			...
			<c:if test="${f.type == 'select'}">
				...
				<div class="field">
					<form:select path="fields[${indx}].strVal" items="${f.list}"
						cssClass="${f.cssClass}" />
					<form:errors path="fields[${indx}].strVal" cssClass="error" />
				</div>
				<div class="clear"></div>
			...
			<c:if test="${f.type == 'checkbox'}">
				...
				<div class="field">
					<form:checkboxes path="fields[${indx}].strVal" items="${f.list}"
						cssClass="${f.cssClass}" />
					<form:errors path="fields[${indx}].strVal" cssClass="error" />
				</div>
				<div class="clear"></div>
			...
			<c:if test="${f.type == 'radio'}">
				...
				<div class="field">
					<form:radiobuttons path="fields[${indx}].strVal" items="${f.list}"
						cssClass="${f.cssClass}" />
					<form:errors path="fields[${indx}].strVal" cssClass="error" />
				</div>
				<div class="clear"></div>
			...
			<c:if test="${f.type == 'file'}">
				...
				<div class="field">
					<input type="file" name="fields[${indx}].byteVal"
						class="${f.cssClass}" />
					<form:errors path="fields[${indx}].byteVal" cssClass="error" />
					<br>
					<a
						href="<c:url value="/formBuilder/dloadTemplate?formId=${formCmd.entryId}"/>">${f.strVal}</a>
				</div>
				<div class="clear"></div>
			</c:if>
		...
		<div class="buttonDivLeft">
			<c:if test="${formAction == 'formDesign'}">
				<input type="button" value="<fmt:message key='button.back'/>"
					class="cancel" onclick="window.location='formList.htm'" />
			</c:if>
		</div>
