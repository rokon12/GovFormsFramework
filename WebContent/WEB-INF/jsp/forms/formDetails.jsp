...
<script type="text/javascript">
	...
	function getFileName() {
		document.forms[0].elements["templateFileName"].value = document.forms[0].elements["pdfTemplate"].value;
		document.forms[0].elements["logoName"].value = document.forms[0].elements["logo"].value;
		return true;
	}
	...
	function deleteLogo(formId) {
		if (confirm("<fmt:message key='msg.confirm'/>")) {
			window.location = "removeLogo.htm?formId=" + formId;
		}
	}
...
<body>
	...
	<form:form modelAttribute="formDetailsCmd" method="POST"
		action="${formDetailsUrl}" id="form1" enctype="multipart/form-data">
		...
		<div class="label">
			<form:label path="detail">
				<fmt:message key="label.logo" />
			</form:label>
		</div>
		<div class="field">
			<input type="file" name="logo" class="" />
			<form:errors path="logo" cssClass="error" />
			<br /> <a href="dloadLogo.htm?formId=${formDetailsCmd.formId}">${formDetailsCmd.logoName}</a>
			<c:if test="${!empty formDetailsCmd.logoName}">
				<br />
				<a href="#" onclick="deleteLogo('${formDetailsCmd.formId}');"><fmt:message
						key="label.remove" /></a>
			</c:if>
		</div>
		...
		<div class="label">
			<form:label path="pdfTemplate">
				<fmt:message key="label.template.file" />
			</form:label>
		</div>
		<div class="field">
			<input type="file" name="pdfTemplate" class="" />
			<form:errors path="pdfTemplate" cssClass="error" />
			<br /> <a href="dloadTemplate.htm?formId=${formDetailsCmd.formId}">${formDetailsCmd.templateFileName}</a>
			<c:if test="${!empty formDetailsCmd.templateFileName}">
				<br />
				<a href="#" onclick="deleteTemplate('${formDetailsCmd.formId}');"><fmt:message
						key="label.remove" /></a>
			</c:if>
		</div>
		<div class="clear"></div>
		...
		<div class="buttonDiv">
			<c:if
				test="${formDetailsCmd.formId == null  && formDetailsCmd.status == 0}">
				<input type="submit" value="<fmt:message key='button.submit'/>"
					onclick="return getFileName();" />
			</c:if>
			<c:if
				test="${formDetailsCmd.formId != null && formDetailsCmd.status > 0}">
