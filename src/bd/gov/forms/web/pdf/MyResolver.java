		...
		try {
			...
			if (systemId.contains("strict.dtd")) {
				dtd = (request)
						...
						.getServletContext()
						.getRealPath(
								"/xhtml_validation_dtd_ent/DTD/xhtml1-strict.dtd");
			} else if (systemId.contains("transitional.dtd")) {
				dtd = (request)
						.getSession()
						.getServletContext()
						.getRealPath(
								"/xhtml_validation_dtd_ent/DTD/xhtml1-transitional.dtd");
			} else if (systemId.contains("xhtml-lat1.ent")) {
				dtd = (request)
						.getSession()
						.getServletContext()
						.getRealPath(
								"/xhtml_validation_dtd_ent/ent/xhtml-lat1.ent");
			} else if (systemId.contains("xhtml-symbol.ent")) {
				dtd = (request)
						.getSession()
						.getServletContext()
						.getRealPath(
								"/xhtml_validation_dtd_ent/ent/xhtml-symbol.ent");
			} else if (systemId.contains("xhtml-special.ent")) {
				dtd = (request)
						.getSession()
						.getServletContext()
						.getRealPath(
								"/xhtml_validation_dtd_ent/ent/xhtml-special.ent");
			}
